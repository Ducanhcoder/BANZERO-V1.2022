...
def countdown(t):
    ...
    while True:
        if (until - datetime.datetime.now()).total_seconds() > 0:
            stdout.flush()
            stdout.write("\r "+Fore.MAGENTA+"[Send]"+Fore.GREEN+" Attacking " + str((until - datetime.datetime.now()).total_seconds()) + " ")
        else:
            stdout.flush()
            stdout.write("\r "+Fore.MAGENTA+"[*]"+Fore.GREEN+" Atacking send done !                                   \n")
            return
#ua
